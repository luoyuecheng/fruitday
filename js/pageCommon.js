...
$(function(){
  ...
  //加载头尾
  $('#headerBox').load('../data/pageCommon.html #header');
  $('#footerBox').load('../data/pageCommon.html #footer');
  $('#zhezhaoBox').load('../data/pageCommon.html #zhezhao');
  $('#broadside_buttonBox').load('../data/pageCommon.html #broadside_button',function(){
  ...
  $.ajax({
    ...
    success:function(res){
      ...
      $('li:has(ul)').children('a').click(function(event){
...
function loginWhether(){
  ...
  for(var i in cookieUser){
    if(cookieUser[i]['status']==1){
      //当前用户为登录状态，则改变头部
      $('#header').find('.pullRight').children('li').eq(0).html('欢迎<a javascript:void(0)>用户'+cookieUser[i]['user']+'</a> , <a href="javascript:void(0)" class="tuichu">退出</a>');
